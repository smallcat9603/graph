...
def printUsage():
    print('Usage: python3 {0} [-m] [-g $graphbase] [-p $nprocesses] [-t $maxthreads] [-c $chunksize] <number_of_servers> <server_id>'.format(os.path.basename(__file__)))
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hmg:p:t:c:") # opts = [("-h", " "), ("-m", " "), ("-g", "..."), ("-t", "...")], args = [number_of_servers, server_id]
    ...
    obj = rw.Walker(this, graph, nprocesses, max_threads, chunk_size, route_table, node_map, hosts)
    ...
    print(f"ncpus = {os.cpu_count()}, nprocesses = {nprocesses}, max_threads = {max_threads}, chunk_size = {chunk_size}")
    daemon.requestLoop()
