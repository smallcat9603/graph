...
class Walker(object):
    ...
    def nexthop(self, cur_local, cur_global):
        neighbors_in = self.graph.vs[cur_local].neighbors()
        ...
        next_idx = random.randint(nneighbors-1)
        next_local_node = -1
        next_global_node = -1
    ...
    def walk(self, message): 
        ...
        next_global_server = -1
        if message[-1] == "go":
            start_local = random.randint(self.graph.vcount()-1)
            start_global = self.map_node[start_local]
            next_local_node, next_global_node, next_global_server = self.nexthop(start_local, start_global)
            message = [start_global]
            message.append(next_global_node)
        else:
            while next_global_server == -1 and len(message) < self.nhops:
                cur_global = message[-1]
                cur_local = self.node_map[cur_global]
                next_local_node, next_global_node, next_global_server = self.nexthop(cur_local, cur_global)
                message.append(next_global_node)
            if len(message) >= self.nhops:
                print("Walker stopped at Server{0}".format(self.name))
                print("Walker walks through nodes: {0}".format(message))
            elif next_local_node == -1:
                print("Walker walks through nodes: {0}".format(message))
